...
function BudgetsPage() {
  ...
  const save = async (category: string) => {
    ...
    const existing = byCat[category];
    if (existing) {
      const { error } = await supabase.from("budgets").update({ amount: v }).eq("id", existing.id);
      if (error) { toast.error(error.message); return; }
    } else {
      const { error } = await supabase.from("budgets").insert({ user_id, category, amount: v });
      if (error) { toast.error(error.message); return; }
    }
